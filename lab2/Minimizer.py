class Minimizer:
    ...
    def method_karnaugh(self):
        ...
        def minimize(target_value):
            terms = [format(i, f'0{n}b') for i, v in enumerate(self.vector) if v == target_value]
            if not terms:
            ...
            def term_to_expr(term, is_dnf):
                ...
                for i, ch in enumerate(term):
                    if ch == '-': continue
                    var = self.variables[i]
                    if is_dnf: parts.append(var if ch == '1' else f"!{var}")
                    else: parts.append(var if ch == '0' else f"!{var}")
                if not parts: return "1" if is_dnf else "0"
                if is_dnf: return " & ".join(parts)
                else: return "(" + " | ".join(parts) + ")"

            sep = " | " if target_value == 1 else " & "
            return sep.join(term_to_expr(t, target_value == 1) for t in essential)

        n = len(self.variables)
        
        dnf, knf = "Не поддерживается", "Не поддерживается"
        
        if n < 2 or n > 5:
            print("Карта Карно: поддержка только 2–5 переменных")
            return dnf, knf
        ...
        if n == 5:
            row_vars, col_vars, extra_var = self.variables[:2], self.variables[2:4], self.variables[4]
        else:
            row_vars, col_vars, extra_var = self.variables[:n//2], self.variables[n//2:], None

        row_gray, col_gray = gray_code(len(row_vars)), gray_code(len(col_vars))

        if not extra_var:
            print("    ", "  ".join(col_gray))
            for r in row_gray:
                row = [str(self.vector[int(r + c, 2)]) for c in col_gray]
                print(f"{r} |  " + "  ".join(row))
        else:
            for e in ['0', '1']:
                print(f"\nСлой {extra_var} = {e}")
                print("    ", "  ".join(col_gray))
                for r in row_gray:
                    row = [str(self.vector[int(r + c + e, 2)]) for c in col_gray]
                    print(f"{r} |  " + "  ".join(row))
